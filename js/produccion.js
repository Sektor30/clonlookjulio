      ...
      function actualizarBarraProgreso(porcentaje) {
          ...
          for (let i = 0; i < usuarios.length; i++) {
              if (usuarios[i].logged) {
                  if (!usuarios[i].progresoProduccion || usuarios[i].progresoProduccion < porcentaje) {
                      usuarios[i].progresoProduccion = Math.floor(porcentaje);
                      localStorage.setItem("usuarios", JSON.stringify(usuarios));
                  }
              }
          }
          // Actualizar barra de la card lateral si existe
          const barraCard = document.getElementById('progress-produccion-card');
          if (barraCard) {
          ...
          const barraDashboard = document.getElementById('progress-produccion-dashboard');
          if (barraDashboard) {
